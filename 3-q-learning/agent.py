...
class QLearning(object):
    ...
    # SARSA传入的参数为s、a、r、s、a
    def S_update(self, state, action, reward, next_state, next_action, done):
        ############################## Q表格的更新方法（需要完成）##################################
        Q_predict = self.Q_table[state][action]
        if done: Q_target = reward
        else: Q_target = reward + self.gamma*(self.Q_table[next_state][next_action])
        # 更新Q表格
        self.Q_table[state][action] += self.lr*(Q_target-Q_predict)
